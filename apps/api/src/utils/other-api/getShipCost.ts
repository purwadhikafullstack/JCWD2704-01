...
export async function getShipCost(
  query: z.infer<typeof rajaOngkirCostQuerySchema>,
) {
  ...
  const convert = async () => {
    ...
    if (!destination)
      throw new NotFoundError('not found destination address address id');
    return {
      ...query,
      origin,
      destination,
  ...
  const fetchRajaOngkirCost = async (
    params: z.infer<typeof rajaOngkirCostParamSchema>,
  ) => {
    try {
      ...
      const data = await rajaOngkirInstance.post<TRajaOngkirCostResponse>(
        '/cost',
        { ...params },
      );
      return data;
      ...
      throw new InternalServerError(
        'RajaOngkir: ' + a.response.data.rajaongkir.status.description,
      );
    }
  };
