...
export async function verifyAdminRefreshToken(
  ...
  try {
    ...
    req.user = verifiedAdminID as TUser;
    next();
  } catch (error) {
    next(error);
  }
...
export async function authorizeStoreAdmin(
  ...
  try {
    if (req.user.role === Role.customer)
      throw new AuthError('Unauthorized access to admin privileges.');
    next();
  } catch (error) {
    next(error);
  }
...
export async function validateStoreAdminDetails(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
export async function validateStoreAdminAddress(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    const validateAddress =
      storeAdminAddressSchema.safeParse(storeAdminAddress);
    if (!validateAddress.success)
      throw new ZodError(validateAddress.error.errors);
...
export async function validateStoreAdminUpdateDetails(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
export async function validateStoreAdminUpdateAddress(
  req: Request,
  res: Response,
  next: NextFunction,
) {
