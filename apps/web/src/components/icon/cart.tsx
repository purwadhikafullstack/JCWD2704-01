...
import LinkUserState from "../linkUserState";
import axiosInstance from "@/lib/serverAxiosInstance";

export default function CartIcon() {
  return (
    <LinkUserState href="/cart" classname="w-max h-max">
      <div className="relative aspect-square h-12 rounded-full p-3 hover:bg-gray-200 hover:shadow-md">
        <svg
          width="full"
          height="full"
          viewBox="0 0 24 24"
        ...
        <CountCart className="absolute right-0 top-0 flex aspect-square items-center rounded-full bg-red-500 px-2 font-bold text-white" />
      </div>
    </LinkUserState>
  );
}

async function CountCart({ className = "" }) {
  //   const { count } = (await axiosInstance().get("/cart/count")) as {count: number};
  return (
