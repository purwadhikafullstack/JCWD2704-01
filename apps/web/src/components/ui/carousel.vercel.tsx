...
const SliderMainItem = forwardRef<HTMLDivElement, {} & React.HTMLAttributes<HTMLDivElement>>(({ className, children, ...props }, ref) => {
  const { orientation } = useCarousel();
  return (
    <div
      {...props}
      ref={ref}
      className={cn(`min-w-0 shrink-0 grow-0 basis-full bg-background p-1 ${orientation === "vertical" ? "pb-1" : "pr-1"}`, className)}
    >
      {children}
    </div>
  );
...
>(({ className, index, children, ...props }, ref) => {
  ...
  return (
    ...
    >
      <div className={`relative aspect-square h-20 w-full rounded-md opacity-40 transition-opacity ${isSlideActive ? "!opacity-100" : ""}`}>
        {children}
      </div>
    </div>
  );
  ...
  ({ className, index, children, ...props }, ref) => {
    ...
    return (
      <Button
        ref={ref}
        size="icon"
        className={cn("h-1 w-6 rounded-full", "data-[active='false']:bg-primary/50 data-[active='true']:bg-primary", className)}
