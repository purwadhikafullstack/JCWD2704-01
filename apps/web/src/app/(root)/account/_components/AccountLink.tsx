    ...
    <ul className="w-full">
      {links.map(({ href, description, icon, label }) => (
        <React.Fragment key={label}>
          <li className="rounded-md bg-background transition-all duration-200 hover:bg-secondary active:scale-[.99]">
            <Link key={label} href={href} className="flex items-center justify-between px-2 py-4">
