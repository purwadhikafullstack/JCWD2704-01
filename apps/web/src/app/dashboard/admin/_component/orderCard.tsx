  ...
  return (
    <>
      ...
      <CardContent className="flex justify-between">
        {order.status !== "canceled" && order.status !== "sended" && <OrderStatusButton inv={order.inv_no} to="cancel" />}
        {order.status == "process" && <OrderStatusButton inv={order.inv_no} to="send" />}
        {order.status == "wait_for_confirmation" && <OrderStatusButton inv={order.inv_no} to="send" />}
        {order.status == "wait_for_payment" && <OrderStatusButton inv={order.inv_no} to="approve payment" />}
      </CardContent>
    </>
  );
}
